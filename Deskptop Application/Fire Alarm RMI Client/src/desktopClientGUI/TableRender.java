...
class TableRender extends DefaultTableCellRenderer {
	...
	public Component getTableCellRendererComponent(JTable table, Object value, boolean isSelected, boolean hasFocus,
		...
		if (((String) table.getValueAt(row, 1)).equals("Not Active")) {
			...
			setBackground(
					Color.getHSBColor(backgroudnColorHSBPink[0], backgroudnColorHSBPink[1], backgroudnColorHSBPink[2]));
		} else if ((Integer) table.getValueAt(row, 5) < 5 && (Integer) table.getValueAt(row, 4) < 5) {
			setForeground(Color.black);
			setBackground(Color.getHSBColor(backgroudnColorHSBGreen[0], backgroudnColorHSBGreen[1],
					backgroudnColorHSBGreen[2]));
		} else if ((Integer) table.getValueAt(row, 5) > 5 || (Integer) table.getValueAt(row, 4) > 5) {
			setBackground(
					Color.getHSBColor(backgroudnColorHSBRed[0], backgroudnColorHSBRed[1], backgroudnColorHSBRed[2]));
			setForeground(Color.black);
		} else if ((Integer) table.getValueAt(row, 5) == 5 || (Integer) table.getValueAt(row, 4) == 5) {
			setForeground(Color.black);
			setBackground(Color.getHSBColor(backgroudnColorHSBYello[0], backgroudnColorHSBYello[1],
					backgroudnColorHSBYello[2]));
		}
